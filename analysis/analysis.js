...
(async function () {
  ...
  async function initBackendImport(bloggerData, userId) {
    ...
    // 渲染后台数据图表（如果有），单独 try-catch 避免阻塞其他功能
    if (backendCount > 0) {
      try {
        renderBackendKPICards(notes);
        renderBackendCharts(notes);
        enhanceTopNoteTabs(notes);
      } catch (err) {
        console.error('[BackendImport] chart render failed:', err);
      }
    }
  }
  ...
  function renderBackendKPICards(notes) {
    ...
    const avgWatch = watchTimes.length > 0 ? watchTimes.reduce((s, v) => s + v, 0) / watchTimes.length : null;

    const ctrColor = avgCTR == null ? '#999' : avgCTR > 0.05 ? '#16a34a' : avgCTR > 0.03 ? '#d97706' : '#dc2626';

    const kpiHtml = `
      <div class="card backend-card">
        <div class="card-value">${fmtNum(totalImpressions)}</div>
        <div class="card-label">总曝光</div>
        <div class="card-label-sub">创作者后台</div>
      </div>
      <div class="card backend-card">
        <div class="card-value" style="color:${ctrColor}">${avgCTR != null ? (avgCTR * 100).toFixed(1) + '%' : '—'}</div>
        <div class="card-label">平均封面点击率</div>
        <div class="card-label-sub">创作者后台</div>
      </div>
      <div class="card backend-card">
        <div class="card-value">${fmtNum(totalFollowerGain)}</div>
        <div class="card-label">总涨粉</div>
        <div class="card-label-sub">创作者后台</div>
      </div>
      <div class="card backend-card">
        <div class="card-value">${fmtNum(totalShares)}</div>
        <div class="card-label">总分享</div>
        <div class="card-label-sub">创作者后台</div>
      </div>
      <div class="card backend-card">
        <div class="card-value">${avgWatch != null ? avgWatch.toFixed(1) + 's' : '—'}</div>
        <div class="card-label">平均观看时长</div>
        <div class="card-label-sub">创作者后台</div>
      </div>
    `;
    cardsContainer.insertAdjacentHTML('beforeend', kpiHtml);
  }

  function renderBackendCharts(notes) {
    const backendNotes = notes.filter(n => n.backend);
    if (backendNotes.length === 0) return;

    // 转化漏斗
    renderFunnelChart(backendNotes);
    // CTR 分布
    renderCTRDistribution(backendNotes);
    // 涨粉归因 + 分享排行
    renderAttributionCharts(backendNotes);
  }

  function renderFunnelChart(backendNotes) {
    const panel = document.getElementById('backend-funnel-panel');
    const el = document.getElementById('backend-funnel-chart');
    if (!panel || !el) return;
    ...
    chart.setOption({
      ...
      color: ['#ff2442', '#ff6b6b', '#ffa94d', '#51cf66'],
      series: [{
        type: 'funnel',
        left: '10%', width: '80%', top: 20, bottom: 20,
        min: 0, max: totalImpressions || 1,
        sort: 'descending', gap: 4,
        label: { show: true, position: 'inside', fontSize: 13 },
        data: [
          { name: '曝光', value: totalImpressions },
          { name: '观看', value: totalViews },
          { name: '互动', value: totalEngagement },
  ...
  function renderCTRDistribution(backendNotes) {
    const panel = document.getElementById('backend-ctr-panel');
    const el = document.getElementById('backend-ctr-chart');
    if (!panel || !el) return;

    const ctrs = backendNotes.map(n => n.backend.coverCTR).filter(v => v != null);
    if (ctrs.length < 3) return;
    panel.classList.remove('hidden');

    // 分桶: 0-5%, 5-10%, 10-15%, 15-20%, 20-25%, 25-30%, 30%+
    const buckets = ['0-5%', '5-10%', '10-15%', '15-20%', '20-25%', '25-30%', '30%+'];
    const counts = new Array(7).fill(0);
    for (const ctr of ctrs) {
      const pct = ctr * 100;
      const idx = Math.min(Math.floor(pct / 5), 6);
      counts[idx]++;
    }
    ...
    chart.setOption({
      ...
      yAxis: { type: 'value', name: '笔记数' },
      series: [{
        type: 'bar', data: counts,
        itemStyle: {
          color: (p) => p.dataIndex <= 1 ? '#fca5a5' : p.dataIndex <= 3 ? '#fcd34d' : '#86efac',
          borderRadius: [4, 4, 0, 0],
        },
  ...
  function renderAttributionCharts(backendNotes) {
    const panel = document.getElementById('backend-attribution-panel');
    if (!panel) return;

    const hasFollower = backendNotes.some(n => n.backend.followerGain > 0);
    const hasShares = backendNotes.some(n => n.backend.shares > 0);
    if (!hasFollower && !hasShares) return;
    panel.classList.remove('hidden');

    // 涨粉 Top 10
    if (hasFollower) {
      const el = document.getElementById('backend-follower-chart');
      if (el) {
        const sorted = [...backendNotes]
          .sort((a, b) => (b.backend.followerGain || 0) - (a.backend.followerGain || 0))
          .slice(0, 10);
        const chart = echarts.init(el);
        chart.setOption({
          title: { text: '涨粉 Top 10', textStyle: { fontSize: 13 } },
          tooltip: { trigger: 'axis' },
          xAxis: { type: 'value' },
          yAxis: {
            type: 'category',
            data: sorted.map(n => truncate(n.title, 12)).reverse(),
            axisLabel: { fontSize: 11 },
          },
          series: [{
            type: 'bar', data: sorted.map(n => n.backend.followerGain || 0).reverse(),
            itemStyle: { color: '#6366f1', borderRadius: [0, 4, 4, 0] },
            label: { show: true, position: 'right', fontSize: 11 },
          }],
          grid: { left: 110, right: 50, top: 35, bottom: 10 },
        });
        new ResizeObserver(() => chart.resize()).observe(el);
      }
    }
    ...
    // 分享 Top 10
    if (hasShares) {
      const el = document.getElementById('backend-share-chart');
      if (el) {
        const sorted = [...backendNotes]
        ...
        const chart = echarts.init(el);
        chart.setOption({
          title: { text: '分享 Top 10', textStyle: { fontSize: 13 } },
          tooltip: { trigger: 'axis' },
          xAxis: { type: 'value' },
          yAxis: {
            type: 'category',
          ...
          series: [{
            type: 'bar', data: sorted.map(n => n.backend.shares || 0).reverse(),
  ...
  function enhanceTopNoteTabs(notes) {
    const tabBar = document.getElementById('tab-bar');
    if (!tabBar) return;
    // 检查是否已添加
    if (tabBar.querySelector('[data-sort="impressions"]')) return;

    const backendTabs = [
      { sort: 'impressions', label: '按曝光' },
      { sort: 'shares', label: '按分享' },
      { sort: 'followerGain', label: '按涨粉' },
    ];
    for (const t of backendTabs) {
      const btn = document.createElement('button');
      btn.className = 'tab';
      btn.dataset.sort = t.sort;
      btn.textContent = t.label;
      btn.addEventListener('click', () => {
        tabBar.querySelectorAll('.tab').forEach(b => b.classList.remove('active'));
        btn.classList.add('active');
        renderTopNotesByBackend(notes, t.sort);
      });
      tabBar.appendChild(btn);
    }
  }

  function renderTopNotesByBackend(notes, sortKey) {
    const backendNotes = notes.filter(n => n.backend);
    const sorted = [...backendNotes]
      .sort((a, b) => (b.backend?.[sortKey] || 0) - (a.backend?.[sortKey] || 0))
      .slice(0, 10);

    const labelMap = { impressions: '曝光', shares: '分享', followerGain: '涨粉' };
    const maxVal = sorted[0]?.backend?.[sortKey] || 1;

    const container = document.getElementById('top-notes');
    if (!container) return;
    container.innerHTML = sorted.map((n, i) => {
      const val = n.backend?.[sortKey] || 0;
      const pct = Math.round(val / maxVal * 100);
      return `
        <div class="bar-item" ${n.noteUrl ? `onclick="window.open('${esc(n.noteUrl)}','_blank')"` : ''} style="cursor:pointer">
          <div class="bar-rank">${i + 1}</div>
          <div class="bar-content">
            <div class="bar-title">${esc(n.title || '无标题')}</div>
            <div class="bar-fill" style="width:${pct}%"></div>
          </div>
          <div class="bar-value">${fmtNum(val)} ${labelMap[sortKey] || ''}</div>
        </div>
      `;
    }).join('');
  }
