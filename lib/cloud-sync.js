...
const CLOUD_SYNC = {
  ...
  async syncAll(progressCallback) {
    ...
    let pull = { newBloggers: 0, updatedBloggers: 0, newNotes: 0, newAiReports: 0 };
    try {
      pull = await this.pullAll(progressCallback);
    } catch (e) {
      console.error('[CloudSync] pull failed:', e);
      errors.push({ userId: '_pull', nickname: '拉取云端', error: e.message });
    }
    ...
    return { pull, bloggerCount, totalNotes, aiReportCount, errors };
  },

  // ========== 拉取：云端 → 本地（仅补全缺失数据，不覆盖本地）==========
  async pullAll(progressCallback) {
    const session = await this._ensureValidSession();
    if (!session) {
      return { newBloggers: 0, updatedBloggers: 0, newNotes: 0, newAiReports: 0 };
    }

    const [cloudBloggers, cloudNotes, cloudAiReports] = await Promise.all([
      this._fetchAllRowsWithSession(session, 'bloggers'),
      this._fetchAllRowsWithSession(session, 'notes'),
      this._fetchAllRowsWithSession(session, 'ai_reports'),
    ]);

    if (cloudBloggers.length === 0 && cloudAiReports.length === 0) {
      return { newBloggers: 0, updatedBloggers: 0, newNotes: 0, newAiReports: 0 };
    }

    const notesByBloggerId = new Map();
    for (const note of cloudNotes) {
      if (!notesByBloggerId.has(note.blogger_id)) {
        notesByBloggerId.set(note.blogger_id, []);
      }
      notesByBloggerId.get(note.blogger_id).push(note);
    }

    const index = await DATA_STORE.getBloggerIndex();
    const existingIds = new Set(index.map((b) => b.userId));
    const writes = {};
    let newBloggers = 0;
    let updatedBloggers = 0;
    let newNotes = 0;

    for (let i = 0; i < cloudBloggers.length; i += 1) {
      const cloudBlogger = cloudBloggers[i];
      progressCallback?.({
        phase: 'pull',
        current: i + 1,
        total: cloudBloggers.length,
        bloggerName: cloudBlogger.nickname,
      });

      const xhsUserId = cloudBlogger.xhs_user_id;
      if (!xhsUserId) continue;

      const cloudNotesForBlogger = (notesByBloggerId.get(cloudBlogger.id) || [])
        .map((row) => this._cloudNoteToLocal(row))
        .filter(Boolean);

      const existingLocal = await DATA_STORE.getBloggerData(xhsUserId);
      const existingNotes = existingLocal?.notes || [];
      const mergedNotes = existingNotes.length
        ? DATA_STORE.mergeNotes(existingNotes, cloudNotesForBlogger, false)
        : cloudNotesForBlogger;

      const addedNotes = mergedNotes.length - existingNotes.length;
      if (existingLocal && addedNotes === 0) continue;
      if (!existingLocal && mergedNotes.length === 0) continue;

      const cloudStats = {
        followerCount: cloudBlogger.follower_count ?? null,
        followingCount: cloudBlogger.following_count ?? null,
      };
      const stats = DATA_STORE._mergeBloggerStats(existingLocal?.stats || null, cloudStats);

      const totalLikes = mergedNotes.reduce((s, n) => s + (n.likes || 0), 0);
      const totalCollects = mergedNotes.reduce((s, n) => s + (n.collects || 0), 0);
      const totalComments = mergedNotes.reduce((s, n) => s + (n.commentCount || 0), 0);

      const nickname = existingLocal?.nickname || cloudBlogger.nickname || '';
      const avatar = existingLocal?.avatar || cloudBlogger.avatar || '';
      const profileUrl =
        existingLocal?.profileUrl ||
        cloudBlogger.profile_url ||
        (/^[a-f0-9]+$/i.test(xhsUserId) ? `https://www.xiaohongshu.com/user/profile/${xhsUserId}` : '');
      const scrapedAt =
        existingLocal?.scrapedAt ||
        (cloudBlogger.scraped_at ? Date.parse(cloudBlogger.scraped_at) : Date.now());

      writes[`blogger_${xhsUserId}`] = {
        userId: xhsUserId,
        nickname,
        avatar,
        profileUrl,
        ...
        stats,
        snapshots: existingLocal?.snapshots || [],
      };
      ...
      const entry = {
        ...
        likedAndCollectedCount: stats?.likedAndCollectedCount ?? null,
        redId: stats?.redId ?? null,
        desc: stats?.desc ?? null,
        gender: stats?.gender ?? null,
        ipLocation: stats?.ipLocation ?? null,
      };

      if (existingIds.has(xhsUserId)) {
        const idx = index.findIndex((b) => b.userId === xhsUserId);
        index[idx] = entry;
        updatedBloggers += 1;
      } else {
        index.unshift(entry);
        existingIds.add(xhsUserId);
        newBloggers += 1;
      }
      newNotes += Math.max(0, addedNotes);
    }

    if (Object.keys(writes).length > 0) {
      writes.bloggerIndex = index;
      await chrome.storage.local.set(writes);
    }

    let newAiReports = 0;
    if (cloudAiReports.length > 0) {
      const keys = cloudAiReports
        .filter((r) => r.scope_key && r.content)
        .map((r) => `aiReport_${r.scope_key}`);
      if (keys.length > 0) {
        const localReports = await chrome.storage.local.get(keys);
        const aiWrites = {};
        for (const row of cloudAiReports) {
          if (!row.scope_key || !row.content) continue;
          const key = `aiReport_${row.scope_key}`;
          if (localReports[key]) continue;
          aiWrites[key] = row.content;
          newAiReports += 1;
        }
        if (Object.keys(aiWrites).length > 0) {
          await chrome.storage.local.set(aiWrites);
        }
      }
    }

    return { newBloggers, updatedBloggers, newNotes, newAiReports };
  },

  _cloudNoteToLocal(row) {
    if (!row?.xhs_note_id) return null;
    const ts = row.publish_time ? Date.parse(row.publish_time) : null;
    return {
      noteId: row.xhs_note_id,
      title: row.title || '',
      content: row.content || '',
      ...
      publishTime: row.publish_text || '',
      publishTimestamp: Number.isFinite(ts) ? ts : null,
